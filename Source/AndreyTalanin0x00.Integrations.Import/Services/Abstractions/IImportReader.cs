...
public interface IImportReader<TImportRequest, TImportResponse>
    where TImportRequest : ImportRequest
    where TImportResponse : ImportResponse
{
    public Task<ImportSourceBatch[]> ReadAsync(TImportRequest importRequest, CancellationToken cancellationToken = default);
}
